...
**类型说明**：
- **训练内**（In-distribution）：模型训练时见过相同行为分布的数据，评估模型在已知分布上的分类能力
- **训练外**（Out-of-distribution）：模型训练时从未接触过此类高抓挠比例场景，评估模型在真实皮肤病犬只分布下的泛化能力（S4/S5 更贴近生产环境实际情况，因此重点关注）
...
> ⚠️ **待真实数据集验证**：以下结果基于合成 IMU 数据，真实标注数据就绪后需重新运行 `run_evaluation.py --fresh` 以替换本节指标。
...
> ⚠️ **待真实数据集验证**：精确率、召回率、F1 均基于合成数据计算，真实数据就绪后需重新验证。
...
> ⚠️ **待真实数据集验证**：混淆矩阵基于合成数据，真实数据下的误分类模式可能与此不同。
...
> ⚠️ **待真实数据集验证**：特征重要性排名基于合成数据训练的模型，真实数据重训后排名可能变化。
...
> ⚠️ **重要说明（测试方请注意）**：第一章（1.5～1.8）所有模型评估指标均基于合成 IMU 数据，合成数据已参照真实设备采集规律校准，但并不等同于真实标注数据。**上线前必须使用真实标注数据集重新运行评估并替换本报告中的模型指标。**

**后续必做**：收集真实标注数据后运行 `python tests/run_evaluation.py --fresh` 更新模型评估指标；积累历史数据后补充评估和基线写入验证。
